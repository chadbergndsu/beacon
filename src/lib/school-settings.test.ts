...
const state = vi.hoisted(() => ({
  settings: {
    brand: { shortName: 'Pilot' },
    modules: { cameras: [] as unknown[] },
...
describe('mergeSchoolSettings', () => {
  ...
  it('nested badge merge keeps sibling keys', async () => {
    state.settings = { badge: { notifyParentsOnAftercare: true, other: 1 }, brand: {} }
