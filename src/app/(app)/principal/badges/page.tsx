...
export default async function PrincipalBadgesPage() {
  ...
  try {
    ...
    ;[badges, rooms, scans, openAftercare] = await Promise.all([
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground leading-relaxed">
          ...
          aftercare sessions can create invoices for payments.
        </p>
      </div>
      ...
      {setupError && (
        <div className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
          ...
          <p className="mt-2 text-xs">
            ...
            <code className="rounded bg-white px-1">scripts/pending-011-badge-kiosk.sql</code> →
            Run → refresh this page.
          </p>
        </div>
      )}
