...
- **Billing money:** first-class tables only (`billing_*`, plans/schedules, `quickbooks_connections`, **`snack_*` LBC wallets**) — not `schools.settings.billing`. Family portal is school-owned (no BillerGenie).
